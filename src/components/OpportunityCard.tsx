...
import { useTaxStore } from "@/contexts/TaxStoreContext";
...
const OpportunityCard = ({ opportunity, asset }: OpportunityCardProps) => {
  ...
  const { harvestedOpportunities, harvestOpportunity } = useTaxStore();
  
  const isHarvested = harvestedOpportunities.includes(opportunity.id);
  ...
  const handleHarvestClick = () => {
    ...
    setTimeout(() => {
      ...
      harvestOpportunity(opportunity.id, asset.name, opportunity.potentialSavings);
    }, 1500);
  };
